import Image from "next/image";
import { Card, Button } from "react-bootstrap"


function DepoimentosMain() {
  return (

    <>
      <container className="depoimentos-principal">
        <div className="depoimentos-container-main">

          <div className="depoimentos-photos">
            <Image src="/static/images/depo_3.png" width="100%" height="100%" />
          </div>

          <div className="depoimentos-text">
            <p>Gostaria de compartilhar o quanto tenho estado feliz com a oportunidade deste voluntariado. Iniciei as atividades de voluntário com o coração desejoso por ajudar e tenho vivido dias em que a experiência mais me motiva e me ensina do que talvez eu possa estar ajudando. Me envolvi com os trabalhos da ONG não apenas por identificação de ideias mas por ver, e hoje mais do nunca, sentir, o que esse trabalho faz pelas pessoas. A proposta de falar sobre a paz ser possível e acessível à todos e todas fortifica os sentimentos de esperança e ajuda em meu coração.
              <br></br>
              <b>Eduardo/Voluntário</b>
            </p>
          </div>

        </div>

        <container className="depoimentos-container-main">

          <div className="depoimentos-photos">
            <Image src="/static/images/depo_4.png" width="100%" height="100%" />
          </div>

          <div className="depoimentos-text">
            <p>O Programa de Educação para a Paz não é um milagre, é uma forma muito simples e eficaz que mostra que as ferramentas que precisamos para ter paz já estão dentro de nós. Apenas precisamos escolher utilizá-las. Tal qual a paz, o voluntariado é uma jornada. E o benefício se espalha como uma onda: você ajuda pessoas e a o mesmo tempo você acaba se ajudando. Nas palavras do poeta Kabir, citado por Prem Rawat “Todos sabemos eu existe uma gota no oceano, mas pouquíssimos sabem que existe um oceano na gota”. Seja gota, seja oceano, lembre-se de que cada um de nós  pode transformar e juntos podemos mais.
              <br></br>
              <b>Susanne/Facilitadora</b>
            </p>
          </div>
        </container>
      </container>

      {/* depoimentos solo */}

      <container className="depoimentos-principal-solo">
        <div className="depoimentos-container-main-solo">

          <div className="depoimentos-photos-solo">
            <Image src="/static/images/depo_5.png" width="100%" height="100%" />
          </div>

          <div className="depoimentos-text-solo">
            <p>Hoje, lembrando os primeiros dias do nosso voluntariado, e contabilizando tudo, percebo a riqueza que essa atitude trouxe a minha vida e também para todos os que vivem em torno de mim. Temos dois aspectos de evolução que acontece: um interno e outro externo.
              Primeiro você entende a dimensão dessa Paz que te inunda as horas, você passa a respirar essa paz, e ela impregna seus pensamentos e suas atitudes! E depois que você se alista neste voluntariado da Paz, você é, e se sente um ser humano muito melhor, com maior consciência, maior apreciação da vida a cada dia!
              Porém, é quando você vê a mudança na conduta do outro, aquele onde a paz chega e chacoalha tudo, com crescimento interior e que se irradia para o exterior, aí você se vê, se identifica nessa mesma humanidade, e se alegra por ter-se permitido essa doação de si.
              Nesse voluntariado, nós vemos a alegria de partilhar ao invés de acumular, num desejo do bem, da fraternidade e da ternura que clamam em nosso coração para se manifestar.

              <br></br>
              <b>Cleide/Voluntária</b>
            </p>
          </div>

        </div>

      </container>

      {/* segunda parte */}
      <container className="depoimentos-principal">
        <div className="depoimentos-container-main">

          <div className="depoimentos-photos">
            <Image src="/static/images/depo_6.png" width="100%" height="100%" />
          </div>

          <div className="depoimentos-text">
            <p>A Paz, seguramente uma das causas mais antigas.
              Por isso, ter a Palavras de Paz em nossa Rede Social é, primeiro, uma honra! Para nós do Instituto União Solidária que promovemos a cultura do trabalho em rede desde 2005, uma organização que promova essa causa de forma estruturada, e com paixão fortalece muito nosso trabalho. Nossa colaboração tem gerado ainda mais impacto positivo!


              <br></br>
              <b>Regina Vizoso/Parceira/Instituto União Solidária</b>
            </p>
          </div>

        </div>

        <container className="depoimentos-container-main">

          <div className="depoimentos-photos">
            <Image src="/static/images/depo_7.png" width="100%" height="100%" />
          </div>

          <div className="depoimentos-text">
            <p>Como voluntária sou testemunha de que o Programa de Educação Para Paz é capaz de modificar a vida das pessoas. Observo que a esperança em si mesmo se renova a cada curso. O programa traz acolhimento, amorosidade, reflexão, compartilhamento e fortalece o  trabalho em equipe. Parabéns a todos os voluntários que fazem a diferença a cada dia!

              <br></br>
              <b>Diva/Voluntária</b>
            </p>
          </div>
        </container>
      </container>

      {/* depoimentos solo */}

      <container className="depoimentos-principal-solo">
        <div className="depoimentos-container-main-solo">

          <div className="depoimentos-photos-solo">
            <Image src="/static/images/depo_8.png" width="100%" height="100%" />
          </div>

          <div className="depoimentos-text-solo">
            <p>O Programa da Paz é fundamental para a sociedade em si, aborda temas essências para a humanidade, ou seja, para todos os seres humanos terem o conhecimento, compreensão do valor da vida, do valor de se ter paz interior, da dignidade com nossos direitos básicos, do nosso mundo interior onde devemos ser donos, reis do nosso castelo,  da apreciação da felicidade nas coisas mais simples, do poder de escolha para as sementes boas da vida, da consciência de si mesmo, sendo gentis conosco primeiro para podermos ser gentis com o próximo,  nós fazemos ter consciência, contentamento, compreensão de que todos nós temos lados bons e lados ruim mas com clareza em busca de manter o equilibro  da força interior e entendimento para buscar as coisas boas da vida, independente da situação, ou seja hoje pode estar chovendo mas amanhã o sol novamente vem para brilhar, não parar. Tudo é essencial para nosso crescimento pessoal, onde estamos em constante evolução podendo mudar e esperança de obter sucesso nas nossas realizações.
              <br></br>
              <b>Pollyanna/Participante</b>
            </p>
          </div>

        </div>

      </container>











    </>

  )
}

export default DepoimentosMain
